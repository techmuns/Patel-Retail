...
## 27. Dark-store cannibalisation score — the fund has no separate formula, so this is the synthesis

§26 shipped the dark-store distance/count stat but deliberately withheld a composite score, since the fund said on the call the score should be "built on your algorithm" — theirs, not invented here. The user asked the fund directly what that algorithm is (weights, thresholds, brand handling, refresh cadence, output shape, decision threshold). **Answer: there isn't one.** The fund's own reply, relayed by the user: they don't have a formula to hand over — they just read the same metrics already in this dashboard (the Excel financials, dark-store proximity, DMart/peer benchmarking data) together, without a fixed weighting. There is nothing further to wait on here.

That changes what this block is. It's not a placeholder standing in for a real formula anymore — it's this project's own numeric combination of the same signals the fund already looks at, still not something the fund has reviewed or endorsed as correct. Computed with `overlapRisk()` from `geo.js` verbatim — the exact 60%-proximity/40%-density weights already trusted for the store-vs-store `overlap_risk` score above it — fed dark-store distances instead of competitor-store distances. One judgment call: density is counted within **1 km**, not the 3 km the store-network score uses, since quick-commerce delivery radii are themselves ~1-2 km and 3 km would saturate every store in a dense town to the same reading. Sanity-checked against all 44 locatable stores before shipping (scores spread 0.00–0.86, avg 0.42 — not saturated at either end).

Rendered as its own `data-kind="estimate"` block (distinct from the `derived` risk score above it), titled "Quick-commerce cannibalisation — our combined read," stating plainly that the fund confirmed they have no separate formula and hasn't reviewed this specific number — framed as a discussion point for the next client meeting, not a finding to present as settled.

The user's stated plan going forward: get the full dashboard live and complete first, then hold a meeting with the fund to work through tweaks — this section, and the kind-label sweep below, are part of "complete."

## 28. Closing out the standing audit — terminology, real peer data, and one shared P&L formula

Several rounds of live-site screenshots from the user surfaced issues beyond what had been tracked, closed out alongside the remaining P2/P3 audit items:

**"The client" meant Patel Retail throughout the app — backwards from the actual client's own vocabulary.** The user's own messages call their fund "my client" ("I'll ask my client"), so a dashboard line like "this is what the client currently does by hand" is genuinely ambiguous to the person reading it. Swept every user-facing occurrence (screen text, PDF, Excel, `metrics.json` notes) to "Patel Retail" or "the two source files" — left code comments alone since those aren't shown to anyone. One Excel-specific instance ("pending client pins") had its own separate hardcoded string and was missed by the first grep pass for "the client"/"client's own" — caught on a second sweep.

**DMart, Vishal Mega Mart, and Spencer's Retail got real financials.** The genuine peer table showed "Not available" for all three despite each being a listed company with disclosed results — researched and cross-verified against independent news sources (Spencer's numbers came back inconsistent across two fetch passes at first; resolved by confirming they were two different fiscal years, not an error, before trusting either).

**The Excel export's Proximity sheet repeated a full explanatory sentence in every one of ~1,400 rows** — "Unavailable — one or both stores have no coordinates at all yet (pending client pins...)" verbatim, hundreds of times. Cut to a short per-row status with the explanation stated once in a legend row under the header. The Unit Economics sheet's notes column had no `wrapText`, so long notes were overflowing across empty neighbouring columns instead of wrapping — looked "unfilled" to the right of column C. Both fixed; verified by generating the actual workbook in a headless browser and reading the buffer back with ExcelJS, not just reading source.

**Kind-label sweep finished.** `map.js` and `economics.js` already had comprehensive coverage from earlier rounds. The real gaps: `screener.js`'s go/no-go read (a threshold classification with zero provenance marker, same shape as §-current `estate.js`'s saturation read) and `peers.js`'s "Patel's own contested figures" table (the only table in the app with no reported/derived pills at all).

**Revenue derivation unified.** `economics.js`, `patel-report.js`, and `peers.js` each had their own copy of the three-line store P&L formula (revenue → gross profit → EBITDA). `patel-report.js`'s copy carried a comment explaining why it wasn't importing from `economics.js` — DOM-wired module, same reasoning `geo.js` already exists for. Extracted the identical pattern into a new `public/js/pnl.js`, pure and dependency-free; all three now import it. `patel-export-xlsx.js` keeps its own step-rounded version — documented explicitly why: its cached cell values must match Excel's own `ROUND()`-per-step formulas on open, which `pnl.js`'s full-precision output doesn't guarantee bit-for-bit.

## 29. Uran resolved as a real 54th store — independent verification, not client cooperation

The user asked directly: with neither the fund nor Patel Retail reachable for the Uran question or further store-level data, what independent options exist?

**Answer, and it changed the store count.** Patel Retail turns out to be a listed company (ticker `PATELRMART`, a Screener.in page) — a fact §21 had flagged as "genuinely unresolved," now settled. Multiple independent press outlets (The Tribune, Loktej, ScanX, The Business Guardian) report that Patel Retail formally disclosed, under **SEBI Regulation 30** (a mandatory exchange filing), opening its **53rd store at Karanja Road, Uran** on **27 Jul 2026** — the exact address already flagged as unmatched since §23. Not a warehouse: the company's actual warehouses are separately named (Ambernath MIDC, Dudhai Gujarat) in the same coverage.

Re-ran `scripts/fetch-official-stores.mjs` against the live `patelrpl.in/stores/` to double-check before acting: still 53 listings, Uran still resolves to a real, high-confidence coordinate (18.871596, 72.941254), with **zero** stores in `stores.json` left unmatched to a site listing — meaning the 53 stores already in the file account for 52 of the site's 53 listings, and Uran is a genuine 54th, not a duplicate or a naming mismatch.

**Added as store `URN`**, following the exact process already established for company-website-sourced coordinates (`geo_source: "company_website"`, `geocode_match_tier: "client"`) — but with one addition: a `source_note` field (new, present only on this record) stating plainly that this store's entire record — not just its coordinate — comes from the official website and independent press, not from `Patel_Retail_data_Munshot.xlsx` like the other 53, and has not been confirmed directly by Patel Retail to this review. `map.js`'s store sheet surfaces this note prominently (a `flag-card`, `data-kind="reported"`, titled "Not from the client's own store file") whenever it's present — the one store where a reader needs to see the provenance is different, not something to leave to a data field nobody reads.

Regenerated `proximity.json` (54 stores, 1431 pairs, 990 with a real distance) via the existing script — no manual edits. Then swept every hardcoded "53" across the app: some were user-facing (the map's "53-Store Network" heading, the reset button's tooltip, the area-estimate caption, the Osia/V2 scale-comparison subtitle on screen and in the PDF) and are now computed live from `stores.length`/`operational.length` rather than hardcoded, so this can't go stale the same way again; code comments were updated for accuracy but left as prose, not dynamic. One figure deliberately did **not** change: `cross_file_contradictions.store_count` in `metrics.json`, which quotes the two *client* files' own internal disagreement (49 vs. 52/53) as a historical fact about those documents — unrelated to this dashboard's own current total.

Verified end to end in a headless browser: KPI strip reads 54 total / 53 operational / 1 closed, Towns Covered sums correctly (Thane 49 + Raigad 4 + Palghar 1 = 54, Raigad up from 3 since Uran is there), the peer table and PDF both show 53 operational stores for the Patel/Osia/V2 scale comparison, and clicking through to Uran's own store sheet (a genuine click-through, not just a data check — the first attempt gave a false negative because the sheet modal from the previous click was still open and blocking subsequent clicks, a test-script bug, not an app bug) shows the `source_note` flag card, correct locality/opened-date, and a risk score computed from real proximity data (nearest own store 26.54 km away — Uran is genuinely isolated from the rest of the network, which is itself a small piece of corroborating geographic sense).

**Still open:** Patel Retail has not confirmed this directly. The `source_note` says so on the store's own sheet — worth raising with them at the same time as the other open items (annual sales/store totals, upcoming store locations, other store-level data).
