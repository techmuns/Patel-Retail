...
## 27. Placeholder cannibalisation score for the dark-store overlay — ours, explicitly not the fund's

§26 shipped the dark-store distance/count stat but deliberately withheld a composite score, since the fund said on the call the score should be "built on your algorithm" — theirs, not invented here. The user asked their fund what that algorithm actually is (its weights, thresholds, which brands count, refresh cadence, output shape, and the threshold that would actually resolve their stated worry) and is waiting on a reply. Rather than block the rest of the feature on that reply, this round adds a **clearly-labelled placeholder** so the store sheet isn't left with a bare distance stat in the meantime.

The placeholder reuses `overlapRisk()` from `geo.js` verbatim — the exact 60%-proximity/40%-density weights already trusted for the store-vs-store `overlap_risk` score above it — fed dark-store distances instead of competitor-store distances. One judgment call was made choosing the inputs: density is counted within **1 km**, not the 3 km the store-network score uses, since quick-commerce delivery radii are themselves ~1-2 km and 3 km would saturate every store in a dense town to the same reading. Sanity-checked against all 44 locatable stores before shipping (scores spread 0.00–0.86, avg 0.42 — not saturated at either end).

Rendered as its own `data-kind="estimate"` block (distinct from the `derived` risk score above it), titled "Quick-commerce cannibalisation — placeholder score," with the sentence **"not the fund's own methodology — that formula has been requested and is pending"** stated directly in the UI, not just in this doc. The moment the fund's real formula arrives, this block is what gets replaced — the distance/count stats and the third-party-data caveats stay as-is either way.
