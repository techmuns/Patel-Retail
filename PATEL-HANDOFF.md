...
| File | Source | Notes |
| --- | --- | --- |
| `public/data/stores.json` | `stores.seed.json` (attached) | All 53 stores, verbatim. `lat`/`lng` still null. |
...
| `public/data/proximity.json` | Generated by `scripts/build-proximity.mjs` | Town clusters are fully populated (no geocoding needed for those — they come straight from each store's `town` field). Pairwise distances and per-store nearest-own/overlap-risk are all `null` until `scripts/geocode.mjs` actually runs — **it hasn't**, because this session has no `GOOGLE_MAPS_API_KEY`. See the script's own header comment for what's needed to run it for real. |

**Still open, matching §12:** run `scripts/geocode.mjs` with a real Google Geocoding key, re-run `scripts/build-proximity.mjs` afterward, then the map will actually show pins instead of its empty state. Hand-verify the three low-confidence rows once geocoded, per §7.
